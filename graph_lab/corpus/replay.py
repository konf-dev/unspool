"""Corpus replayer — feeds JSONL through the graph pipeline."""
...
async def _graph_chat(
    ...
    """Process a single message through the graph pipeline. Returns (response, timing_dict)."""
...
async def replay_corpus(
    ...
    Unspool only sees msg.content — the raw text. Corpus metadata (energy, mood,
    scenario_tag) is preserved in ReplayTurn as ground truth for evaluation only.
    """
    persona = corpus_path.stem
    if user_id is None:
        user_id = f"replay-{persona}-{uuid4().hex[:8]}"

    console.print(f"[bold]Replaying:[/bold] {corpus_path.name} → user {user_id}")
    if skip_feedback:
    ...
    # Incremental output — write each turn as JSONL so progress survives crashes
    _RESULTS_DIR.mkdir(exist_ok=True)
    incremental_path = _RESULTS_DIR / f"{user_id}_turns.jsonl"
    ...
    try:
        ...
        with open(corpus_path) as corpus_f, open(incremental_path, "w") as inc_f:
            for line in corpus_f:
                ...
                if data.get("type") == "day_marker":
                    ...
                    # Run evolution AFTER previous day's messages (matches simulate.py)
                ...
                # Corpus message — Unspool only sees .content
                msg = CorpusMessage(**data)
                result.total_messages += 1
                day_had_messages = True

                try:
                    response, timings = await _graph_chat(user_id, msg.content, skip_feedback)
                except Exception as e:
                ...
                turn = ReplayTurn(
                    ...
                    graph_stats=stats,
                    # Ground truth — for evaluation, not seen by Unspool
        ...
        # Final evolution after last day
        if day_had_messages:
            await evolve_graph(user_id)
            result.evolutions_run += 1
        ...
        console.print(
            ...
            f"python -m graph_lab.cli inspect --user-id {user_id}[/dim]"
        )
    finally:
        await db.close()
...
async def evaluate_replay(result: ReplayResult) -> ReplayEvaluation:
    """Evaluate replay quality using corpus ground truth.

    The evaluator LLM sees both sides: what the user said AND the hidden
    metadata (mood, energy, scenario intent). Unspool never saw the metadata —
    so this measures how well the system inferred things from raw text alone.
    """
    ...
    # Build transcript with ground truth annotations
    ...
    # Score specific scenario types
    scenario_scores = await _score_scenarios(result.turns, persona_desc, eval_model, eval_temp)

    # Overall assessment
    assessment = await _overall_assessment(transcript, persona_desc, scores, eval_model, eval_temp)
...
async def _score_scenarios(
    ...
    """Score how well Unspool handled each scenario type."""
    # Group turns by scenario tag
    ...
    for tag, tagged_turns in scenario_turns.items():
        # Build mini-transcript for this scenario
        lines = []
...
def _build_annotated_transcript(turns: list[ReplayTurn]) -> str:
    """Build transcript with hidden ground truth annotations for the evaluator."""
    lines = []
    current_day = 0
    for t in turns:
        ...
        lines.append(f"[{t.time_of_day}] [hidden: energy={t.energy}, mood={t.mood}]{tag}")
...
def _save_results(result: ReplayResult) -> None:
    """Save replay result as JSON + markdown transcript."""
    _RESULTS_DIR.mkdir(exist_ok=True)

    # JSON
    json_path = _RESULTS_DIR / f"{result.user_id}.json"
    json_path.write_text(result.model_dump_json(indent=2))

    # Markdown transcript
    md_path = _RESULTS_DIR / f"{result.user_id}_transcript.md"
...
def _format_transcript_md(result: ReplayResult) -> str:
    """Human-readable markdown transcript with ground truth annotations."""
    lines = [
