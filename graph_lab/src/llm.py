...
from graph_lab.src.config import (
    LLM_API_KEY,
    LLM_MODEL,
    OLLAMA_URL,
    OPENAI_API_KEY,
...
def _route(model: str) -> str:
    """Determine provider: 'anthropic', 'openai', or 'ollama'."""
    if model.startswith("claude"):
        return "anthropic"
    if model.startswith(("gpt-", "o1", "o3", "text-embedding")):
        return "openai"
    # Everything else (qwen, llama, mistral, etc.) → Ollama
    return "ollama"
...
async def generate(
    ...
    if provider == "anthropic":
        return await _anthropic_generate(messages, model, system, temperature, max_tokens)
    if provider == "ollama":
...
async def generate_json(
    ...
    if provider == "ollama":
        return await _ollama_generate_json(messages, model, system, temperature, max_tokens)
...
async def stream(
    ...
    if provider == "anthropic":
        async for token in _anthropic_stream(messages, model, system, temperature, max_tokens):
            yield token
    elif provider == "ollama":
        async for token in _ollama_stream(messages, model, system, temperature, max_tokens):
            yield token
    else:
        async for token in _openai_stream(messages, model, system, temperature, max_tokens):
            yield token
...
async def _ollama_generate_json(
    ...
    """Ollama supports JSON mode natively via response_format."""
    client = _get_ollama()
    msgs = _build_openai_messages(messages, system)
    response = await client.chat.completions.create(
        model=model,
