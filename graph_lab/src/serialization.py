...
from graph_lab.src.types import ActiveSubgraph, Node
...
def serialize_subgraph(subgraph: ActiveSubgraph, user_message: str) -> str:
    """Convert active subgraph to LLM-readable context text."""
    config = load_graph_config()
    max_tokens = config.serialization.max_context_tokens
    ...
    for edge in subgraph.edges:
        ...
        # SurrealDB uses 'in' and 'out' for relation direction
        if hasattr(edge, "model_extra") and edge.model_extra:
            from_id = edge.model_extra.get("in", from_id)
            to_id = edge.model_extra.get("out", to_id)
    ...
    suppressed = _find_suppressed(subgraph, node_map, outgoing)
    recent = _find_recent(node_map)

    # Build sections with priority-based truncation
    now = datetime.now(timezone.utc)
    sections: list[tuple[str, int]] = []  # (text, priority)

    # Always include: current time
    time_header = f"Current time: {now.strftime('%Y-%m-%d %I:%M %p')} ({now.strftime('%A')})"

    # Section: Open items (high priority)
    if open_items:
    ...
    # Section: Schedule (high priority)
    ...
    # Section: People (medium priority)
    if people:
        lines = ["PEOPLE:"]
    ...
    # Section: Recently surfaced (high priority — tells LLM what not to repeat)
    if suppressed:
    ...
    # Section: Recent context (low priority)
    if recent:
    ...
    remaining = max_tokens - count_tokens(result_parts[0]) - 20  # buffer for closing tag
...
def _find_open_items(
    ...
    """Find nodes connected to 'not done' with strength > 0."""
    items = []
    not_done_ids = {n.id for n in subgraph.nodes if n.content.lower() == "not done"}
    if not not_done_ids:
        return items

    # Find edges pointing to "not done"
    for edge in subgraph.edges:
        from_id = edge.from_node_id
        to_id = edge.to_node_id
        if hasattr(edge, "model_extra") and edge.model_extra:
...
def _find_date_nodes(node_map: dict[str, Node]) -> dict[str, datetime]:
    """Find nodes whose content is an ISO date."""
    dates = {}
...
def _find_people(
    ...
    """Heuristic: nodes that are proper nouns and connect to many things are likely people."""
    people = {}
    for nid, node in node_map.items():
        content = node.content
        # Skip status nodes, dates, and short content
        if content.lower() in _STATUS_NODES or _ISO_DATE.match(content):
            continue
        # Heuristic: capitalized single word or two words, connected to 3+ things
        words = content.split()
        if len(words) <= 2 and words[0][0].isupper():
            connections = len(outgoing.get(nid, [])) + len(incoming.get(nid, []))
            if connections >= 2:
                # Build brief description from connected nodes
                connected_contents = []
                for cid in (outgoing.get(nid, []) + incoming.get(nid, []))[:3]:
                    if cid in node_map and node_map[cid].content.lower() not in _STATUS_NODES:
                        connected_contents.append(node_map[cid].content)
                desc = ", ".join(connected_contents) if connected_contents else "mentioned"
                people[content] = desc
    return people


def _find_suppressed(
    subgraph: ActiveSubgraph, node_map: dict[str, Node], outgoing: dict[str, list[str]]
) -> list[str]:
    """Find items that were recently surfaced."""
...
def _find_recent(node_map: dict[str, Node]) -> list[str]:
    """Get recently activated non-status nodes."""
    nodes = sorted(
        node_map.values(),
...
def _format_open_item(
    ...
    """Format a single open item with deadline and connection info."""
    parts = [f'"{item.content}"']

    # Check for connected date (deadline)
    for connected_id in outgoing.get(item.id, []):
        if connected_id in date_nodes:
            ...
            if delta.days < 0:
                ...
                parts.append(f"due in {delta.days} days ({deadline.strftime('%Y-%m-%d')})")

    # Check for connected descriptors
    for connected_id in outgoing.get(item.id, []):
...
def _find_schedule(
    ...
    """Find things scheduled for today/tomorrow."""
    items = []
    for nid, dt in sorted(date_nodes.items(), key=lambda x: x[1]):
        delta = dt - now
        if -1 <= delta.days <= 1:
            # Find what's connected to this date
            connected = [
            ...
            if connected:
                ...
                items.append(f"{', '.join(connected)} — {when} ({dt.strftime('%Y-%m-%d')})")
    return items
