...
from graph_lab_sql.src.types import TriggerResult
...
async def trigger_temporal(user_id: str, context: dict, params: dict) -> TriggerResult:
    """Find date nodes within window + their neighbors. Single SQL query."""
...
async def trigger_recent(user_id: str, context: dict, params: dict) -> TriggerResult:
    """Nodes activated in last N hours."""
    ...
    rows = await pool.fetch(
        ...
        "WHERE user_id = $1 AND last_activated_at > $2 "
        "ORDER BY last_activated_at DESC LIMIT $3",
        user_id,
        cutoff,
        limit,
    )
...
async def trigger_walk(user_id: str, context: dict, params: dict) -> TriggerResult:
    """Walk via neighbor cache — no traversal needed."""
