...
from graph_lab_sql.src.config import load_graph_config, load_triggers_config
from graph_lab_sql.src.triggers import get_trigger
from graph_lab_sql.src.types import ActiveSubgraph, Edge, Node, TriggerResult
...
async def build_active_subgraph(
    ...
    # Phase 1: Independent triggers in parallel
    if independent:
        tasks = [
    ...
    # Phase 2: Dependent triggers sequentially
...
def _map_node(n: dict) -> dict:
    """Map asyncpg row dict to Node model fields."""
    mapped = dict(n)
    # Ensure string IDs
    for key in ("id", "user_id", "source_stream_id"):
...
def _map_edge(e: dict) -> dict:
    """Map asyncpg row dict to Edge model fields."""
    mapped = dict(e)
    for key in ("id", "user_id", "from_node_id", "to_node_id", "source_stream_id"):
        if key in mapped and mapped[key] is not None:
            mapped[key] = str(mapped[key])
    return mapped
