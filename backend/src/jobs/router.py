...
from src.telemetry.langfuse_integration import observe, propagate_trace_attributes
...
@observe(name="job.hourly_maintenance")
async def hourly_maintenance() -> dict:
    trace_id = str(uuid.uuid4())
    _log.info("job.start", job="hourly_maintenance", trace_id=trace_id)

    with propagate_trace_attributes(tags=["job", "hourly_maintenance"]):
...
@observe(name="job.nightly_batch")
async def nightly_batch() -> dict:
    trace_id = str(uuid.uuid4())
    _log.info("job.start", job="nightly_batch", trace_id=trace_id)

    with propagate_trace_attributes(tags=["job", "nightly_batch"]):
        results: dict[str, Any] = {}
...
@observe(name="job.process_message")
