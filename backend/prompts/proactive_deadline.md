...
The user just opened the app. They have {{ items | length }} item(s) with deadlines in the next 24 hours.

Items:
{% for item in items %}
- <user_input>{{ item.content }}</user_input> (deadline: {{ item.deadline }})
{% endfor %}
