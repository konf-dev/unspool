...
if [[ "$NO_BACKUP" == false ]]; then
  ...
  # Write manifest alongside the backup
  manifest_file="${backup_file%.sql.gz}.manifest.txt"
  git_sha=$(git rev-parse HEAD 2>/dev/null || echo "unknown")
  git_branch=$(git rev-parse --abbrev-ref HEAD 2>/dev/null || echo "unknown")
  git_dirty=$(git diff --quiet 2>/dev/null && echo "clean" || echo "dirty")
  applied_list=$(psql "$PGURL" -t -A -c "SELECT version FROM public.schema_migrations ORDER BY version;" 2>/dev/null || echo "(table not found)")
  table_list=$(psql "$PGURL" -t -A -c "SELECT tablename FROM pg_tables WHERE schemaname='public' ORDER BY tablename;" 2>/dev/null || echo "(could not query)")
  view_list=$(psql "$PGURL" -t -A -c "SELECT viewname FROM pg_views WHERE schemaname='public' ORDER BY viewname;" 2>/dev/null || echo "(could not query)")

  cat > "$manifest_file" <<MANIFEST
# Backup Manifest
# Generated by migrate.sh — do not edit
...
## Pending Migrations (at time of backup)
$(printf '%s\n' "${PENDING[@]}" | while read -r f; do basename "$f" .sql; done)

## Restore Command
# gunzip -c $backup_file | psql "\$PGURL"
MANIFEST
  echo "Manifest:  $manifest_file"

  # Prune old backups (keep last MAX_BACKUPS — removes both .sql.gz and .manifest.txt)
  backup_count=$(ls -1 "$BACKUP_DIR"/backup_*.sql.gz 2>/dev/null | wc -l)
  if [[ "$backup_count" -gt "$MAX_BACKUPS" ]]; then
    ls -1t "$BACKUP_DIR"/backup_*.sql.gz | tail -n +"$((MAX_BACKUPS + 1))" | while read -r old; do
